...
\mbox{sens} &= \frac{1}{4 \pi H^2}
            ...
            &= \frac{1}{4 \pi H^2}\frac{\pi R^2}{3}
             = \frac{1}{12} \left( \frac{a}{T} \right)^2 \label{eq:collim:sens}
\end{align}
with
\begin{equation}
...
%
\begin{equation}
  R_2  \simeq R_0 e^{- R_0 (\tau_1 + \tau_2)} \label{eq:dead2}
\end{equation}
%
...
E(L_x(X, \Lambda) | Q, \Lambda^{old}) = 
   \sum_i \sum_j ( n_{ij} \ln (c_{ij} \lambda_j) -  c_{ij} \lambda_j) \label{eq:jnestep1}
\end{equation}
%
\begin{equation}
n_{ij} = c_{ij} \lambda_j^{old} \frac{q_i}{\sum_k c_{ik} \lambda_k^{old}} \label{eq:jnestep2}
\end{equation}
%
